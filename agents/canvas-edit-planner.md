...
description: >-
  ...
  presents an edit plan for user approval via a markdown approval prompt; gathers describe_control output
...
- The plugin root path, which contains `agents/`, `skills/`, and `references/`
- The list of synced `.pa.yaml` files

## Step 1 — Read Reference Documents

Use the already loaded `TechnicalGuide.md` and `DesignGuide.md` context before doing anything else.
If they are not already in context, read them from the plugin root's `references/` directory.
These documents govern every YAML syntax and design decision.
...
Present the following to the user as a formatted markdown block:
...
"Please review this plan. Reply **'approved'** to continue, or describe any changes you'd like."

Do not proceed until the user replies with `approved` or an equivalent affirmation.
If they request changes, update the plan and present it again.
If approved: proceed to Step 5.

## Step 5 — Gather Control Property Definitions (If Needed)
...
## Step 6 — Write canvas-edit-plan.md
